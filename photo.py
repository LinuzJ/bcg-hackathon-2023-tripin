...
def photo_generator(location):
    # Define your API key
    api_key = 'AIzaSyAWEPohy9CdHpz6j8-_zLDRsSWoDI9b2YU'
    ...
    if response.status_code == 200:
        ...
        if 'results' in data and len(data['results']) > 0:
            ...
            photo_reference = data['results'][0].get('photos', [])[0].get('photo_reference', None)
            if photo_reference:
                ...
                photo_response = requests.get(photo_endpoint, params=photo_params)
                if photo_response.status_code == 200:
                    ...
                    print(f"Error downloading photo: {photo_response.status_code} - {photo_response.text}")
            else:
                print("No photo_reference found in the search results.")
        else:
            print("No results found for the search query.")
    else:
        print(f"Error searching for places: {response.status_code} - {response.text}")
